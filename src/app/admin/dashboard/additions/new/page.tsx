...
import { createAdditionSchema, type CreateAdditionInput } from "@/lib/validations/addition";

export default function NewAdditionPage() {
  ...
  const onSubmit = async (data: CreateAdditionInput) => {
    setIsSubmitting(true);
    setError(null);

    const addition = await createAddition(data);

    if (addition) {
      router.push("/admin/dashboard/additions");
    } else {
      setError("Error al crear la adición");
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold">Nueva Adición</h2>
          <p className="text-muted-foreground">Crea un nuevo extra para el menú</p>
        </div>
      </div>
